(function () {
    const cursor = {
        ...
        init() {
            this.cursorCreation = document.createElement('div');
            this.cursorCreation.classList.add('cursor');
            this.cursorCreation.innerHTML = `
            ...
            `;
            document.body.appendChild(this.cursorCreation);

            // Mettre à jour les références une fois le DOM mis à jour
            this.cursor = document.querySelector('.cursor');
            this.path = this.cursor.querySelector('path');
            this.svg = this.cursor.querySelector('svg');
            this.elements = document.querySelectorAll('.hoverCursor');
        ...
        addEventListeners() {
            ...
            this.elements.forEach(element => {
                element.addEventListener('mouseenter', () => {
                    this.updateSvgStyle('#cb321e', '30px', '30px');
                });

                element.addEventListener('mouseleave', () => {
                    this.updateSvgStyle('rgba(203,50,30,0.7)', '20px', '20px');
                });
            });
        },

        updateSvgStyle(fillColor, width, height) {
            this.path.style.fill = fillColor;
